...
const phyloxml_js_1 = require("./lib/phyloXml/phyloxml.js");
...
class alcmonavispoeschli {
    constructor() {
        ...
        this.branchLengthScaling = (nodes, width) => {
            ...
            var yScale = d3_1.default.scale.linear()
        ...
        this.calcMaxTreeLengthForDisplay = () => {
            return SettingsDeclared(this.settings) && OptionsDeclared(this.options)
                ? this.settings.rootOffset + this.options.nodeLabelGap + AP.LABEL_SIZE_CALC_ADDITION + (this.maxLabelLength * this.options.externalNodeFontSize * AP.LABEL_SIZE_CALC_FACTOR)
                : 0;
        };
        this.isCanDoMsaResidueVisualizations = () => {
            return SettingsDeclared(this.settings) && ((this.settings.enableNodeVisualizations === true)
                && (this.settings.enableMsaResidueVisualizations === true)
                && (this.basicTreeProperties && this.basicTreeProperties.alignedMolSeqs === true)
                && (this.basicTreeProperties && this.basicTreeProperties.maxMolSeqLength && this.basicTreeProperties.maxMolSeqLength > 1)) || false;
        };
        ...
        // ----------------------------
        this.mouseover = () => {
            this.node_mouseover_div.transition()
                .duration(300)
                .style('opacity', 1);
        };
        this.mousemove = (d) => {
                ...
                if (scaleType === AP.ORDINAL_SCALE) {
                    if (mappingFn.domain() && mappingFn.range() && mappingFn.domain().length > mappingFn.range().length) {
                        if (altMappingFn && altMappingFn.domain() && altMappingFn.range()) {
                            ...
                            console.log(AP.WARNING + ': Ordinal scale mapping for ' + label + ' (' + s + '): domain > range: ' +
        ...
        this.initializeNodeVisualizations = (nodeProperties) => {
            if (this.nodeVisualizations) {
                for (var key in this.nodeVisualizations) {
                    if (this.nodeVisualizations.hasOwnProperty(key)) { // '...iterate over the properties of an object without executing on inherited properties.'
                        ...
                        if (nodeVisualization.label) {
                            ...
                            if (nodeVisualization.shapes &&
                                ...
                                if (nodeVisualization.label === AP.MSA_RESIDUE) {
                                    ...
                                    shapeScale = d3_1.default.scale.ordinal()
                            ...
                            if (nodeVisualization.colors) {
                                ...
                                if ((nodeVisualization.cladeRef && nodeProperties[nodeVisualization.cladeRef] && forester_1.forester.setToArray(nodeProperties[nodeVisualization.cladeRef]).length > 0)
                                    ...
                                    if (Array.isArray(nodeVisualization.colors)) {
                                        ...
                                        if (nodeVisualization.colors.length === 3) {
                                            colorScale = d3_1.default.scale.linear()
                                    ...
                                    if (Array.isArray(nodeVisualization.colorsAlt)) {
                                        if (nodeVisualization.colorsAlt.length === 3) {
                                            altColorScale = d3_1.default.scale.linear()
                                    ...
                                    if (forester_1.isString(nodeVisualization.colors) && nodeVisualization.colors.length > 0) {
                                        scaleType = AP.ORDINAL_SCALE;
                                        if (nodeVisualization.label === AP.MSA_RESIDUE) {
                                            colorScale = d3_1.default.scale.category20()
                                                .domain(this.basicTreeProperties.molSeqResiduesPerPosition[0]);
                            ...
                            if (nodeVisualization.sizes && Array.isArray(nodeVisualization.sizes) && (nodeVisualization.sizes.length > 0)) {
                                if (nodeVisualization.cladeRef && nodeProperties[nodeVisualization.cladeRef] && forester_1.forester.setToArray(nodeProperties[nodeVisualization.cladeRef]).length > 0) {
                                    ...
                                    if (nodeVisualization.sizes.length === 3) {
                                        sizeScale = d3_1.default.scale.linear()
        ...
        this.makeColorLegend = (id, xPos, yPos, colorScale, scaleType, label, description) => {
            ...
            legendUpdate.select('text.' + AP.LEGEND_LABEL)
        ...
        this.makeShapeLegend = (id, xPos, yPos, shapeScale, label, description) => {
            ...
            legendUpdate.select('path')
                ...
                .attr('d', d3_1.default.svg.symbol()
                .size(function () {
                return 20;
            })
        ...
        this.prepareColorsForColorPicker = () => {
            const DEFAULT_COLORS_FOR_COLORPICKER = [
                ...
                // Basic
                '#FFFFFF', '#999999', '#000000', '#FF0000', '#00FF00', '#0000FF', '#FF00FF', '#FFFF00', '#00FFFF', this.options && this.options.backgroundColorDefault
            ].filter(x => x);
        ...
        this.makeColorPicker = (id) => {
            ...
            for (var ii = 0; ii < this.colorsForColorPicker.length; ++ii) {
                lbls[ii] = ii;
                if (clickedOrigColorIndex < 0 && (colorToHex(this.colorsForColorPicker[ii]) === colorToHex(this.colorPickerData.clickedOrigColor))) {
                    clickedOrigColorIndex = ii;
                }
            }
            var colorPickerColors = d3_1.default.scale.linear()
                .domain(lbls)
                .range(this.colorsForColorPicker);
            var colorPickerSize = 14;
            var rectSize = 10;
            var xCorrectionForLabel = -1;
            var yFactorForDesc = -0.5;
            var colorPicker = this.baseSvg.selectAll('g.' + id)
            ...
            colorPickerUpdate.select('text.' + AP.COLOR_PICKER_LABEL)
                .attr('x', xCorrectionForLabel)
                .attr('y', yFactorForDesc * rectSize)
                .text((d, i) => {
                if (i === 0) {
                    return 'Choose ' + this.colorPickerData.legendLabel.toLowerCase() +
                        ' for ' + this.colorPickerData.legendDescription.toLowerCase() + ' "' +
        ...
        this.update = (source_u, transitionDuration_u, doNotRecalculateWidth) => {
            ...
            node.select("text.extlabel")
                ...
                .attr('dy', (d) => {
                return (d.children || d._children) ? 0.3 * +options.internalNodeFontSize + 'px' : 0.3 * +options.externalNodeFontSize + 'px';
            })
                .attr('x', (d) => {
            ...
            node.select('circle.nodeCircle')
                .attr('r', (d) => {
                if (((options.showNodeVisualizations && !options.showNodeEvents) &&
                ...
                .style('fill', (d) => {
                return (options.showNodeVisualizations || options.showNodeEvents || this.isNodeFound(d)) ? this.makeNodeFillColor(d) : options.backgroundColorDefault;
            });
            ...
            node.each(((self) => {
                const _ = function (d) {
                    if (d._children) {
                        ...
                        d3_1.default.select(this).select('path').transition().duration(transitionDuration || 0)
                            .attr('d', function () {
                        ...
                        d3_1.default.select(this).select('.collapsedText').attr('font-size', function (_d) {
                            return options.externalNodeFontSize + 'px';
                        });
                        d3_1.default.select(this).select('.collapsedText').transition().duration(transitionDuration || 0)
                            .style('fill-opacity', 1)
            ...
            link.exit()
                ...
                .remove();
            if (this.options.phylogram && this.options.alignPhylogram && this.options.showExternalLabels
                && (this.options.showNodeName || this.options.showTaxonomy || this.options.showSequence)) {
                var linkExtension = this.svgGroup.append("g")
                    .selectAll('path')
                    .data(links.filter(function (d) {
                    return (!d.target.children
                        && !(options.dynahide && d.target.hide));
                }));
        ...
        this.makeNodeSize = (node) => {
            if (!OptionsDeclared(this.options))
                throw "Options not set";
            if ((this.options.showNodeEvents && node.events && node.children
                && (node.events.duplications
                    || node.events.speciations))
                || this.isNodeFound(node)) {
                return this.options.nodeSizeDefault;
            }
            return ((this.options.nodeSizeDefault > 0 && node.parent && !(this.options.showNodeVisualizations && node.hasVis))
                && ((node.children && this.options.showInternalNodes)
                    || ((!node._children && !node.children) && this.options.showExternalNodes))
                || (this.options.phylogram && node.parent && !node.parent.parent && (!node.branch_length || node.branch_length <= 0))) ? this.makeVisNodeSize(node, 0.05) : 0;
        };
        ...
        this.makeBranchColor = (link) => {
            if (!OptionsDeclared(this.options))
                throw "Options not set";
            if (!this.visualizations || !this.visualizations.nodeFillColor)
                throw "Node Fill Colour Visualisation not set";
            ...
            if (this.options.showBranchVisualizations && n != null) {
                if ((this.currentLabelColorVisualization === AP.MSA_RESIDUE
                    ...
                    || this.currentNodeFillColorVisualization === AP.MSA_RESIDUE) && this.isCanDoMsaResidueVisualizations()) {
        ...
        this.makeVisNodeFillColor = (node) => {
            if (this.options && this.options.showNodeVisualizations && !node._children && this.currentNodeFillColorVisualization
                && this.visualizations && this.visualizations.nodeFillColor) {
        ...
        this.makeVisNodeSize = (node, correctionFactor) => {
            ...
            if (this.options.showNodeVisualizations && this.currentNodeSizeVisualization) {
                if (this.visualizations && !node._children && this.visualizations.nodeSize
                    && this.visualizations.nodeSize[this.currentNodeSizeVisualization]) {
        ...
        this.calcCollapsedColorInSubtree = (node) => {
            ...
            if (total > 0 && this.foundSum > 0 && OptionsDeclared(this.options)) {
                if ((found0and1 > 0) || ((found0 > 0) && (found1 > 0))) {
                    ...
                    return d3_1.default.scale.linear()
                        .domain([0, total])
        ...
        this.getFoundColor = (phynode) => {
            if (OptionsDeclared(this.options)) {
                if (!this.options.searchNegateResult) {
                    ...
                    if ((this.foundNodes0 && !this.searchBox0Empty) && (this.foundNodes1 && !this.searchBox1Empty) && !this.foundNodes0.has(phynode) && !this.foundNodes1.has(phynode)) {
                        return this.options.found0and1ColorDefault;
                    }
        ...
        this.isNodeFound = (phynode) => {
            if (OptionsDeclared(this.options) && !this.options.searchNegateResult) {
                ...
                if (((this.foundNodes0 && !this.searchBox0Empty) && !this.foundNodes0.has(phynode)) || ((this.foundNodes1 && !this.searchBox1Empty) && !this.foundNodes1.has(phynode))) {
                    return true;
                }
            }
            return false;
        ...
        this.makeNodeLabel = (phynode) => {
            ...
            if (this.options.showTaxonomy && phynode.taxonomies && phynode.taxonomies.length > 0) {
                ...
                if (this.options.showTaxonomySynonyms) { // BM synonymS or synonym ?
        ...
        this.makeCollapsedLabel = (node, descs) => {
            ...
            if (first && last) {
                ...
                if (first_label && last_label) {
                    text = first_label.substring(0, this.options.collapsedLabelLength)
        ...
        this.initializeOptions = (options) => {
            ...
            if (this.options.searchAinitialValue && (typeof this.options.searchAinitialValue === 'string' /*|| this.options.searchAinitialValue instanceof String*/) && this.options.searchAinitialValue.trim().length > 0) {
            ...
            if (this.options.searchBinitialValue && (typeof this.options.searchBinitialValue === 'string' /*|| this.options.searchBinitialValue instanceof String*/) && this.options.searchBinitialValue.trim().length > 0) {
        ...
        this.launch = (id, phylo, options, settings, nodeVisualizations, specialVisualizations) => {
            ...
            if (settings.groupSpecies) {
                if (settings.groupSpecies.source && settings.groupSpecies.target) {
                    console.log(AP.MESSAGE + ' Grouping species from \"' + settings.groupSpecies.source
            ...
            if (settings.groupYears) {
                if (settings.groupYears.source && settings.groupYears.target && settings.groupYears.ignore && settings.groupYears.groupsize) {
                    console.log(AP.MESSAGE + ' Grouping years from \"' + settings.groupYears.source
            ...
            if (settings.enableNodeVisualizations) {
                if (settings.enableMsaResidueVisualizations && (this.basicTreeProperties.alignedMolSeqs === true)
                    && (this.basicTreeProperties.maxMolSeqLength && this.basicTreeProperties.maxMolSeqLength > 1)) {
                ...
                if (this.settings && this.settings.dynamicallyAddNodeVisualizations === true) {
                    ...
                    refsSet.forEach((value) => {
                        ...
                        if (propertyName && this.settings && (!this.nodeVisualizations.hasOwnProperty(propertyName))
                            &&
                                (!this.settings.propertiesToIgnoreForNodeVisualization || (this.settings.propertiesToIgnoreForNodeVisualization.indexOf(propertyName) < 0))) {
            ...
            this.createGui();
            if (settings.enableNodeVisualizations || settings.enableBranchVisualizations) {
                d3_1.default.select(window)
                    .on("mousedown", this.mouseDown);
            }
            this.baseSvg = d3_1.default.select(id).append('svg')
            ...
            this.treeFn = d3_1.default.layout.cluster()
            ...
            if (this.options && this.options.initialCollapseFeature) {
                ...
                if (this.depth_collapse_level >= max_depth) {
                    console.log(AP.WARNING + ' initial value for collapse depth [' + this.depth_collapse_level + '] is larger than or equal to maximum depth [' + max_depth + ']');
        ...
        this.parsePhyloXML = (data) => {
            var phy = phyloxml_js_1.phyloXml.parse(data, { trim: true, normalize: true })[0];
            forester_1.forester.addParents(phy);
            return phy;
        };
        this.parseNewHampshire = forester_1.forester.parseNewHampshire;
        this.calcMaxExtLabel = () => {
                ...
                const _ = function () {
                    ...
                    function displayNodeData(n) {
                        ...
                        $('.ui-dialog').css({
                            ...
                            'font-size': fs,
                            'font-family': (self.settings && self.settings.controlsFont || []).map(v => /\s/.test(v) ? '"' + v + '"' : v).reduce((p, v) => p + ', ' + v),
                            'font-style': 'normal',
                            'font-weight': 'normal',
                    ...
                    var rectHeight = 230;
                    self.removeTooltips();
                    d3_1.default.select(this).append('rect')
                    ...
                    d3_1.default.select(this).append('text')
                        ...
                        d3_1.default.select(this).append('text')
                            ...
                            if (d.sequences) {
                                for (var i = 0; i < d.sequences.length; ++i) {
                                    ...
                                    if (s.accession && s.accession.value && s.accession.source) {
                                        var source = s.accession.source.toUpperCase();
                                        if (source === AP.ACC_GENBANK || source === AP.ACC_REFSEQ || source === AP.ACC_UNIPROT
                    ...
                    d3_1.default.select(this).selectAll('.tooltipElemText').each((() => {
                        const _1 = function (d) {
        ...
        this.updateNodeVisualizationsAndLegends = (tree) => {
            ...
            if (this.showLegends
                ...
                if (this.legendColorScales[AP.LEGEND_NODE_FILL_COLOR] && this.visualizations.nodeFillColor && this.currentNodeFillColorVisualization && this.visualizations.nodeFillColor[this.currentNodeFillColorVisualization]) {
                ...
                if (this.legendColorScales[AP.LEGEND_NODE_BORDER_COLOR] && this.visualizations.nodeBorderColor && this.currentNodeBorderColorVisualization && this.visualizations.nodeBorderColor[this.currentNodeBorderColorVisualization]) {
                ...
                if (this.legendShapeScales[AP.LEGEND_NODE_SHAPE] && this.visualizations.nodeShape && this.currentNodeShapeVisualization && this.visualizations.nodeShape[this.currentNodeShapeVisualization]) {
                    this.removeShapeLegend(AP.LEGEND_NODE_SHAPE);
                    this.addLegendForShapes(AP.LEGEND_NODE_SHAPE, this.visualizations.nodeShape[this.currentNodeShapeVisualization]);
                }
                if (this.legendSizeScales[AP.LEGEND_NODE_SIZE] && this.visualizations.nodeSize && this.currentNodeSizeVisualization && this.visualizations.nodeSize[this.currentNodeSizeVisualization]) {
                    this.removeSizeLegend(AP.LEGEND_NODE_SIZE);
        ...
        this.midpointRootButtonPressed = () => {
            if (this.root && (this.superTreeRoots.length < 1)
                && this.treeData && (this.treeData.rerootable === undefined || this.treeData.rerootable === true)) {
                this.unCollapseAll(this.root);
        ...
        this.search0 = () => {
            ...
            this.searchBox0Empty = true;
            var query = $('#' + AP.SEARCH_FIELD_0).val();
        ...
        this.search1 = () => {
            ...
            this.searchBox1Empty = true;
            var query = $('#' + AP.SEARCH_FIELD_1).val();
            if (query && query.length > 0) {
                var my_query = query.trim();
                if (my_query.length > 0) {
        ...
        this.legendMoveDown = (x) => {
            ...
            if (this.options && this.options.visualizationsLegendYpos && this.options.visualizationsLegendYpos < this.displayHeight) {
                this.options.visualizationsLegendYpos += x;
                this.removeColorPicker();
                this.update(undefined, 0);
            }
        ...
        this.legendMoveRight = (x) => {
            ...
            if (this.options && this.options.visualizationsLegendXpos && this.options.visualizationsLegendXpos < (this.displayWidth - 20)) {
        ...
        this.setRadioButtonValue = (id, value) => {
            var radio = $('input#' + id); // BM
            if (radio) {
                radio[0].checked = value;
                radio.button('refresh');
            }
        };
        this.setCheckboxValue = (id, value) => {
            var cb = $('#' + id);
            if (cb && cb[0]) {
                cb[0].checked = value;
                cb.button('refresh');
        ...
        this.createGui = () => {
            ...
            this.node_mouseover_div = d3_1.default.select("body").append("div")
            ...
            $('#' + AP.LABEL_COLOR_SELECT_MENU).on('change', function () {
                const v = this.value;
            ...
            $('#' + AP.LABEL_COLOR_SELECT_MENU_2).on('change', function () {
                const v = this.value;
            ...
            $('#' + AP.LABEL_COLOR_SELECT_MENU_3).on('change', function () {
                const v = this.value;
                self.setSelectMenuValue(AP.LABEL_COLOR_SELECT_MENU, AP.DEFAULT);
                if (self.isAddVisualization2()) {
                    self.setSelectMenuValue(AP.LABEL_COLOR_SELECT_MENU_2, AP.DEFAULT);
            ...
            $('#' + AP.NODE_FILL_COLOR_SELECT_MENU).on('change', function () {
                var v = this.value;
                if (self.isAddVisualization2()) {
            ...
            $('#' + AP.NODE_FILL_COLOR_SELECT_MENU_2).on('change', function () {
                const v = this.value;
            ...
            $('#' + AP.NODE_FILL_COLOR_SELECT_MENU_3).on('change', function () {
                const v = this.value;
                self.setSelectMenuValue(AP.NODE_FILL_COLOR_SELECT_MENU, AP.DEFAULT);
                if (self.isAddVisualization2()) {
                    self.setSelectMenuValue(AP.NODE_FILL_COLOR_SELECT_MENU_2, AP.DEFAULT);
             ...
             */
            $('#' + AP.NODE_SHAPE_SELECT_MENU).on('change', function () {
                var v = this.value;
                if (v && v != AP.DEFAULT) {
            ...
            $('#' + AP.NODE_SIZE_SELECT_MENU).on('change', function () {
                var v = this.value;
                if (v && v != AP.DEFAULT) {
            ...
            $('#' + AP.MSA_RESIDUE_VIS_CURR_RES_POS_LABEL).keyup((e) => {
                ...
                if ((((keycode >= AP.VK_0) && (keycode <= AP.VK_9)) || ((keycode >= AP.VK_0_NUMPAD)) && (keycode <= AP.VK_9_NUMPAD)) || (keycode === AP.VK_BACKSPACE) || (keycode === AP.VK_DELETE)) {
                    ...
                    if ((((keycode >= AP.VK_0) && (keycode <= AP.VK_9))
                        ...
                        var x = $('#' + AP.MSA_RESIDUE_VIS_CURR_RES_POS_LABEL).val().trim();
                        if (x === '') {
                            return;
                        }
                        i = parseInt(x);
            ...
            function makeAutoCollapse() {
                ...
                if (self.settings && self.settings.enableCollapseByFeature) {
                    ...
                    var refs = self.treeData && forester_1.forester.collectPropertyRefs(self.treeData, 'node', false);
                    if (refs) {
                        refs.forEach(function (v) {
                            var label = v;
                            label = label.replace(/^.+:/, '');
         ...
         * @returns {*}
         */
        this.parseTree = (location, data, newHamphshireConfidenceValuesInBrackets, newHamphshireConfidenceValuesAsInternalNames) => {
            ...
            if (location.substr(-3, 3).toLowerCase() === 'xml') {
                ...
                tree = this.parseNewHampshire(data, newHamphshireConfidenceValuesInBrackets, newHamphshireConfidenceValuesAsInternalNames);
            }
            return tree;
        };
        /**
        ...
        this.launchArchaeopteryx = (label, location, data, options, settings, newHamphshireConfidenceValuesInBrackets, newHamphshireConfidenceValuesAsInternalNames, nodeVisualizations) => {
            var tree = null;
            try {
                tree = this.parseTree(location, data, newHamphshireConfidenceValuesInBrackets, newHamphshireConfidenceValuesAsInternalNames);
            }
            catch (e) {
