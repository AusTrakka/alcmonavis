...
export default class alcmonavispoeschli {
    ...
    update = async (source_u?: Alcmonavis.phylo, transitionDuration_u?: number, doNotRecalculateWidth?: boolean) => {
        ...
        const unsearchedShownNodeNames = unsearchedShownNodes.filter(usn => usn.name !== undefined).map(usn => usn.name as string);
        if (unsearchedShownNodeNames && unsearchedShownNodeNames.length > 0 && settings.searchCallback) {
            ...
            let calls = 0;
            while (unsearchedShownNodeNames.length > 0) {
                const nodesToSearch = unsearchedShownNodeNames.splice(0, settings.callsize);
