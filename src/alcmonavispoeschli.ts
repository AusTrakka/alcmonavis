    ...
    makeNodeLabel = (phynode: Alcmonavis.phylo) => {
        ...
        phynode.properties?.forEach(p => {
            ...
            if (GetType(p.value) === "array") {
                (p.value as any[]).forEach(pv => { // eslint-disable-line @typescript-eslint/no-explicit-any
                    const
                        keyindex = Object.keys(pv)
                            .map((k) => k.toLowerCase())
                            .indexOf('key'),
                        valindex = Object.keys(pv)
                            .map((k) => k.toLowerCase())
                            .indexOf('value');
                    if (keyindex > -1 && valindex > -1 && options.labelCategories && options.labelCategories[pv[Object.keys(pv)[keyindex]]]) {
