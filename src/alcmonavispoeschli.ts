...
const GetType = (x: any): JSType => Object.prototype.toString.call(x).replace(/\[object (.*?)\]/, "$1").toLowerCase() as JSType; // eslint-disable-line @typescript-eslint/no-explicit-any
                            ...
                            if (property.ref && property.value) {
                                ...
                                switch (jsType) {
                                    ...
                                    case "date":
                                        ...
                                        break;
                                    case "object": {
                                        const nameindex = Object.keys(property.value).map(k => k.toLowerCase()).indexOf("name");
                                        if (nameindex > -1) {
                                        ...
                                        for (let j = 0; j < (property.value as any[]).length; ++j) { // eslint-disable-line @typescript-eslint/no-explicit-any
                                            ...
                                            switch (GetType(val)) {
                                                ...
                                                case "date":
                                                    ...
                                                    break;
                                                case "object": {
                                                    const nameindex = Object.keys(val).map(k => k.toLowerCase()).indexOf("name"),
                                                        keyindex = Object.keys(val).map(k => k.toLowerCase()).indexOf("key"),
                                                        valindex = Object.keys(val).map(k => k.toLowerCase()).indexOf("value");
                                                    if (nameindex > -1) {
