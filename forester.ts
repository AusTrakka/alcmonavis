...
export class forester {
     ...
     */
    preOrderTraversal = (node: phylo, fn: voidFn<phylo>) => {
        fn(node);
        if (node.children) {
     ...
     * @param fn - The function to apply.
     */
    preOrderTraversalAll = (node: phylo, fn: voidFn<phylo>) => {
        fn(node);
        if (node.children) {
            for (var i = node.children.length - 1; i >= 0; --i) {
     ...
     * @param fn - The function to apply.
     */
    postOrderTraversalAll = (node: phylo, fn: voidFn<phylo>) => {
        if (node.children) {
     ...
     */
    findByNodeName = (node: phylo, name: string): phylo[] => {
        let found: phylo[] = [];
    ...
    findByTaxonomyCode = (node: phylo, code: string): phylo[] => {
        let found: phylo[] = [];
        this.preOrderTraversalAll(node, function (n) {
    ...
    findByTaxonomyScientificName = (node: phylo, scientificName: string): phylo[] => {
        let found: phylo[] = [];
        this.preOrderTraversalAll(node, function (n) {
     ...
     * @param nodeToDelete
     */
    deleteSubtree = (phy: phylo, nodeToDelete: phylo) => {
        if (!phy) {
        ...
        if (p.children && p.children.length === 1) {
            var pp = p.parent as phylo;
            var cni = this.getChildNodeIndex(pp, p);
            if ((cni < 0) || (cni > ((pp.children as phylo[]).length - 1))) {
     ...
     * non-negative).
     */
    reRoot = (phy: phylo, node: phylo | string, branchLength: number) => {
        if (!phy) {
        ...
        if (!node.parent || !node.parent.parent) {
            ...
            if (node.parent.children && node.parent.children.length > 2) {
                ...
                let prev_root = root;
                (prev_root.children as phylo[]).splice(index, 1);
                let nr = {} as phylo;
                nr.children = [];
            ...
            let a = node;
            let new_root = {} as phylo;
            let distance1;
            let distance2 = 0.0;
            let branch_data_1;
            let branch_data_2 = null;
            let b = a.parent as phylo;
            let c = b.parent as phylo;
            ...
            if ((c.children as phylo[]).length == 2) {
                var node2 = (c.children as phylo[])[1 - this.getChildNodeIndex(c, b)];
                node2.parent = b;
                if ((!c.branch_length)
                    && (!node2.branch_length)) {
                ...
                var l =(b.children as phylo[]).length;
                for (var i = 0; i < l; ++i) {
                    if ((b.children as phylo[])[i] === c) {
        ...
        function setChildNodeOnly(parentNode: phylo, i: number, node: phylo) {
    ...
    getFurthestDescendant = (node: phylo) => {
     ...
     */
    getDistance = (n1: phylo, n2: phylo): number => {
        var d = 0.0;
        while (n1 !== n2) {
            if (!n1.parent) {
    ...
    removeChildNode = (parentNode: phylo, i: number) => {
        if (!parentNode.children) {
     ...
     */
    setChildNode = (parentNode: phylo, i: number, node: phylo) => {
    ...
    setBranchData = function (node: phylo, branchData?: branchData | null) {
        if (branchData) {
    ...
    copyBranchData = (nodeFrom: branchData, nodeTo: branchData) => {
        nodeTo.width = nodeFrom.width;
        nodeTo.color = nodeFrom.color;
        nodeTo.confidences = nodeFrom.confidences;
    };

    getChildNodeIndex = (parentNode: phylo, childNode: phylo): number => {
        if (!parentNode) {
    ...
    getChildren = (node: phylo): phylo[] => {
        return node._children ? node._children : (node.children ? node.children : []);
    };

    calcAverageTreeHeight = (node: phylo, externalDescendants?: phylo[]): number => {
     ...
     * @returns {{}}
     */
    collectProperties = (phy: phylo, appliesTo: properties, externalOnly: boolean): Dict<Set<string>> => {
     ...
     * @returns {boolean}
     */
    splitProperty = (phy: phylo, targetValue: string, fromRef: string, toRef: string): boolean => {
        var found = false;
    ...
    collectPropertyRefs = (phy: phylo, appliesTo: properties, externalOnly?: boolean): Set<string> => {
    ...
    shortenProperties = (phy: phylo, appliesTo: properties, externalOnly: boolean | undefined, sourceRef: string, targetRef: string) => {
        this.preOrderTraversalAll(phy, function (n) {
            if (!externalOnly || externalOnly !== true || (!n.children && !n._children)) {
                if (n.properties && n.properties.length > 0) {
                    var propertiesLength = n.properties.length;
    ...
    collectBasicTreeProperties = (tree: phylo): TreeProperty => {
        let properties = {} as TreeProperty;
    ...
    searchData = (query: string,
        ...
        function matchme(s: string | null | undefined,
        ...
        function makeNDF(query: string): NDF | null {
            var str = query.substring(0, 2) as NDF
            // if (str === "NN"
            //     || str === "TC"
            //     || str === "TN"
            //     || str === "TS"
     ...
     */
    calcSumOfExternalDescendants = (node: phylo): number => {
        var nodes = 0;
     ...
     */
    calcSumOfAllExternalDescendants = (node: phylo): number => {
        var nodes = 0;
        this.preOrderTraversalAll(node, function (n) {
            if (!(n.children || n._children)) {
                ++nodes;
     ...
     * collapsed
     */
    isHasCollapsedNodes = (node: phylo): boolean => {
        var collapsed = false;
        this.preOrderTraversalAll(node, function (n) {
            if (n._children) {
                collapsed = true;
    ...
    getAllExternalNodes = (node: phylo): phylo[] => {
        var nodes: phylo[] = [];
        this.preOrderTraversalAll(node, function (n) {
            if (!n.children && !n._children) {
                nodes.push(n);
    ...
    calcMaxDepth = (node: phylo): number => {
        var max = 0;
    ...
    calcBranchLengthSimpleStatistics = (node: phylo): BranchStats => {
        var stats = {} as BranchStats;
        stats.mean = 0;
    ...
    calcMaxBranchLength = (node: phylo): number => {
        var max = 0;
        this.preOrderTraversalAll(node, function (n) {
    ...
    isHasNodeData = (node: phylo): boolean | undefined => {
    ...
    removeMaxBranchLength = (node: phylo) => {
        this.preOrderTraversalAll(node, function (n) {
            if (n.max) {
                n.max = undefined;
            }
    ...
    collapseToBranchLength = (root: phylo, branchLength: number) => {
        const collapseToBranchLengthHelper = (n: phylo, branchLength: number) => {
            if (!(n.children || n._children)) {
                return;
            }
     ...
     */
    parseNewHampshire = (nhStr: string, confidenceValuesInBrackets: boolean, confidenceValuesAsInternalNames: boolean): phylo => {

        const addConfidence = (x: phylo, element: string) => {
            var confValue = parseConfidence(element);
            if (confValue != null) {
                x.confidences = [];
                var conf = {} as confidence;
        ...
        const moveInternalNodeNamesToConfidenceValues = (node: phylo) => {
            this.preOrderTraversalAll(node, (n) => {
                if (n.children || n._children) {
                    if (n.name) {
                        var s = n.name;
            ...
            if (element === '"' && !in_single_q) {
                ...
                if (in_double_q || in_single_q) {
                    ...
                    if (element === '(') {
                        ...
                        var subtree2 = {} as phylo;
                        (ancs[ancs.length - 1].children as phylo[]).push(subtree2);
                        x = subtree2;
                    }
                    else if (element === ')') {
                        x = ancs.pop() as phylo;
    ...
    isNumber = (v: number | undefined | null): v is number => {
    ...
    getOneDistinctTaxonomy = (node: phylo) => {
    ...
    getOneDistinctNodePropertyValue = (node: phylo, propertyRef: string) => {
     ...
     */
    toNewHampshire = (phy: phylo, decPointsMax?: number, replaceChars?: boolean, writeConfidences?: boolean): string => {
        const toNewHampshireHelper = (node: phylo, last: boolean) => {
            if (node.children) {
                var l = node.children.length;
                nh += "(";
                for (var i = 0; i < l; ++i) {
    ...
    roundNumber = function (num: number, dec: number): number {
        return Math.round(num * Math.pow(10, dec)) / Math.pow(10, dec);
    };

    isString = function (s: phylo | string): s is string {
...
interface phylo extends branchData {
    rooted: boolean;
    name?: string;
...
interface TreeProperty {
    ...
    molSeqResiduesPerPosition?: any[] | null;
